...
var SAMPLE_DATAS = {
		...
		works : [
			{
				...
				plans : [
					...
					},
					{
						...
						label_id   : 7,
						tooltip    : {
							name1    : '1ツールチップツールチップツールチップツールチップツールチップ',
							name2    : '2ツールチップツールチップツールチップツールチップツールチップ',
							name3    : '3ツールチップツールチップツールチップツールチップツールチップ',
						},
					},
				],
				reals : [
	...
	2 : {
		...
		works : [
			...
			{
				id : 23,
				name : 'ライン3＜名称が長い場合の表示テスト＞＜名称が長い場合の表示テスト＞',
				qty1 : '1111',
				qty2 : '2222',
				qty3 : '3333',
				qty4 : '4444',
...
var ViewJobs = function($job, jobs) {
	...
	$.each(jobs, function (idx, v) {
		...
		if(ti != '') {
			let ti_names = [];
			if(v.tooltip.name1) ti_names.push(v.tooltip.name1);
			if(v.tooltip.name2) ti_names.push(v.tooltip.name2);
			if(v.tooltip.name3) ti_names.push(v.tooltip.name3);
			let ti_name = ti_names.join('<br>');
			if(ti_name != '') {
				let ti_url = (! v.tooltip.url)? '': v.tooltip.url;
				let ti_attr = 'class="tooltip" style="left:' + l + 'px;"';
				if(ti_url != '') {
					elm_ti = '<a ' + ti_attr + ' href="' + ti_url + '" target="_blank">' + ti_name + '</a>';
				} else {
					elm_ti = '<span ' + ti_attr + '>' + ti_name + '</span>';
				}
				has_ti = ' has_tooltip';
			}
